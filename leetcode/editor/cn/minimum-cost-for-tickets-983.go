...
// 1 <= costs[i] <= 1000
//
// Related Topics 数组 动态规划 👍 424 👎 0

// 2022-01-15 08:18:48
//leetcode submit region begin(Prohibit modification and deletion)
func mincostTickets(days []int, costs []int) int {
	var n = days[len(days)-1]
	dp := make([]int, n+1)

	find_item := func(d int) bool {
		for _, v := range days {
			if d == v {
				return true
			}
	...
	find_max := func(a, b, c int) int {
		return max(a, max(b, c))
	}

	for i := 0; i < n+1; i++ {
		if find_item(i) {
			dp[i] = find_max(dp[i-1]+costs[0], dp[max(i-7, 0)]+costs[1], dp[max(i-30, 0)]+costs[2])
		} else {
			dp[i] = dp[i-1]
		}
	}
