...
//
//
// 如上所示的一个链接，其域名为 example.org。简单起见，你可以假设所有的链接都采用 http协议 并没有指定 端口。例如，链接 http://
//leetcode.com/problems 和 http://leetcode.com/contest 是同一个域名下的，而链接http://example.
//org/test 和 http://example.com/abc 是不在同一域名下的。
//
// HtmlParser 接口定义如下：
//
...
// 域名标签不会以连字符即减号（‘-’）开头或结尾。
// 关于域名有效性的约束可参考: https://en.wikipedia.org/wiki/Hostname#Restrictions_on_valid_
//hostnames
// 你可以假定url库中不包含重复项。
//
// Related Topics 深度优先搜索 广度优先搜索 字符串 交互 👍 18 👎 0
